...
def lookup_users(user_ids, user_id_url_template, users):
...
'''
        ...
        for media in tweet['extended_entities']['media']:
            if 'url' in media and 'media_url' in media:
                ...
                if os.path.isfile(archive_media_path):
                    ...
                    markdown += '![]({{ "' + f'{new_url}' + '" | relative_url }})'
                    ...
                    if len(archive_media_paths) > 0:
                        for archive_media_path in archive_media_paths:
                            ...
                            markdown += '<video controls><source src="{{ "' + f'{media_url}' +'" | relative_url }}">Your browser does not support the video tag.</video>\n'
    ...
    return timestamp, body_markdown, body_html, tweet_id_str
...
'''
def parse_profile(input_folder):
    requests = import_module('requests')
    sleep_time=0.25
    ...
    with open(f'../_config.yml','a') as profile_config:
        ...
        users = {}
        user_input = input(f'Download additional profile information from Twitter (approx 2.5KB)? [y/n]')
        if user_input.lower() not in ('y', 'yes'):
            return
        ...
        with requests.Session() as session:
            ...
            if users:
                ...
                user = None
            if user:
                print('Saving user profile information to _config.yml...', user)
                if user['id']:
...
def parse_tweets(input_filenames, username, users, archive_media_folder,
   ...
   """
    ...
    current_month = 0
    current_day = 0
    for timestamp, md, html, id in tweets:
...
def parse_followers(data_folder, users, user_id_URL_template, output_followers_filename):
    ...
    lookup_users(follower_ids, user_id_URL_template, users)
    for id in follower_ids:
        handle = users[id].handle if id in users else '~unknown~handle~'
        followers.append('- handle: ' + handle + '\n  twitter_url: ' + user_id_URL_template.format(id))
    followers.sort()
